...
function Loader( editor ) {
	...
	this.loadFiles = function ( files, filesMap ) {
		...
		editor.signals.downloadProgressChanged.dispatch(1);
	...
	this.loadFile = function ( file, manager ) {
		...
		// Downloading progression
		reader.addEventListener( 'progress', function ( event ) {

			editor.signals.readProgressChanged.dispatch(event.loaded / event.total);
	...
	//
...

			{
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					editor.signals.parseProgressChanged.dispatch(0);

					loader.parse( contents, '', function ( result ) {

						setTimeout(() => { editor.signals.parseProgressChanged.dispatch(1); }, 100);

						const scene = result.scene;
						scene.name = filename;

						setTimeout(() => {  editor.signals.renderProgressChanged.dispatch(0.3); }, 100);

						scene.animations.push( ...result.animations );

						setTimeout(() => { editor.signals.renderProgressChanged.dispatch(0.8); }, 100);

						editor.execute( new AddObjectCommand( editor, scene ) );

						setTimeout(() => { editor.signals.renderProgressChanged.dispatch(1); }, 100);
					} );
	...
	async function handleZIP( contents ) {
		...
		if ( zip[ 'model.obj' ] && zip[ 'materials.mtl' ] ) {
			...
			// Bug to import dynamically in Node.js (I have put them at top)
			//const { MTLLoader } = await import( '../jsm/loaders/MTLLoader.js' );
			//const { OBJLoader } = await import( 'three/addons/loaders/OBJLoader.js' );
