...
class UIOutliner extends UIDiv {
	...
	setOptions( options ) {
		...
		function onCheckBoxClick(){
			...
			const object = scope.scene.getObjectById( scope.selectedValue );
			scope.editor.execute( new SetValueCommand( scope.editor, object, 'visible', this.checked ) );
		}
		...
		scope.dom.appendChild(domTable);

		for ( let i = 0; i < options.length; i ++ ) {

			const div = options[ i ];
			div.className = 'option';
			domTable.appendChild( div );
			...
			if (div.getElementsByTagName("input")[0])
				div.getElementsByTagName("input")[0].addEventListener('change', onCheckBoxClick);
...
class UIOutlinerTable extends UIDiv {
	...
	setOptions( options ) {
		...
		domTable.style.borderSpacing = "0";
		...
		td1.textContent = "Elements";
		trHeader.append(td1);
		let td2 = document.createElement('td');
		td2.textContent = "ID";
		trHeader.append(td2);
		let td3 = document.createElement('td');
		td3.textContent = "Color";
		trHeader.append(td3);
		let td4 = document.createElement('td');
		td4.textContent = "Mode";
		trHeader.append(td4);

		domTbody.appendChild( trHeader );
		...
		for ( let i = 0; i < options.length; i ++ ) {
			...
			scope.options.push( tr );

			tr.addEventListener( 'click', onClick );

			if (tr.getElementsByTagName("input")[0])
				tr.getElementsByTagName("input")[0].addEventListener('change', onCheckBoxClick);

			if ( tr.draggable === true ) {

				tr.addEventListener( 'drag', onDrag );
				tr.addEventListener( 'dragstart', onDragStart ); // Firefox needs this
	...
	// 	// }
	//
	// 	// const outlinerHeaderTitle = document.createElement( 'div' );
	// 	// outlinerHeaderTitle.innerHTML = "<div class='outlinerHeaderTitle'>hi</div>";
	// 	// outlinerHeaderTitle.classList.add("outlinerHeaderTitle");
	//
	//
	// 	let domRowHeader = document.createElement( 'tr' );
	// 	// domTable.style.display = "table";
	// 	domRowHeader.innerHTML = innerHTMLvalue;
	//
	// 	//console.log("domTable", domTable);
	//
	// 	this.dom.appendChild(domRowHeader);
	//
	// 	return this;
	// }
}
...
export { UITexture, UICubeTexture, UIOutliner, UIPoints, UIPoints2, UIPoints3, UIBoolean, UIOutlinerTable };
