...
import { SetGeometryBufferCommand } from './commands/SetGeometryBufferCommand.js';
...
function SidebarObject( editor ) {
	...
	*/
	...
	const objectMergeVerticesRow = new UIRow();
	...
	//const objectMergeVertices = new UICheckbox().onChange( update );

	objectMergeVerticesRow.add( new UIText( strings.getKey( 'sidebar/object/mergevertices' ) ).setWidth( '220px' ) );

	const objectGeometryMergeThreshold = new UINumber(1).setPrecision( 3 ).setWidth( '50px' ).onChange( update );
	objectMergeVerticesRow.add(objectGeometryMergeThreshold);

	const objectMergeVertices = new UIButton( "Run" ).onClick( function(){
		editor.execute( new SetGeometryBufferCommand( editor, editor.selected, 'mergeVertices', objectGeometryMergeThreshold ) );
	} ).setWidth( '90px' ).setMarginLeft( '30px' );

	objectMergeVerticesRow.add( objectMergeVertices );

	container.add( objectMergeVerticesRow );
	...
	function update() {
		...
		if ( object !== null ) {
			...
			// if ( object.geometry.mergeVertices !== objectMergeVertices.getValue() ) {
			//
			// 	editor.execute( new SetGeometryBufferCommand( editor, object, 'mergeVertices', objectMergeVertices.getValue() ) );
			//
			// }
