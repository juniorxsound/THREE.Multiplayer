...
class ViewHelper extends THREE.Object3D {
	...
	constructor( editorCamera, dom ) {
		...
		this.render = function ( renderer ) {
			...
			const x = xOffsetLeft; // dom.offsetWidth - dim;

			renderer.clearDepth();

			renderer.getViewport( vpTemp );
			renderer.setViewport( x, yOffsetBottom, dim, dim );
		...
		this.handleClick = function ( event ) {
			...
			// const offsetX = rect.left; // + xOffsetLeft; //( dom.offsetWidth - dim );
			// const offsetY = rect.top; // + ( dom.offsetHeight - dim );
			// mouse.x = ( ( event.clientX - offsetX ) / ( rect.width - offsetY  ) ) * 2 - 1;
			// mouse.y = - ( ( event.clientY - offsetY ) / ( rect.bottom - offsetY ) ) * 2 + 1;


			// const offsetX = rect.left + dom.offsetWidth - dim;
			// mouse.x = ( ( event.clientX - offsetX ) /
			// 			( rect.width    - offsetX ) ) * 2 - 1;


			let b2 = xOffsetLeft + dim;
			let b1 = xOffsetLeft;
			let a = 2/(b2 - b1);
			let b = 1 - 2*b2 / ( b2 - b1 );
			mouse.x =  a*event.x+b;
			...
			b2 = rect.top + dom.offsetHeight - yOffsetBottom - dim;
			b1 = rect.top + dom.offsetHeight - yOffsetBottom;
			a = 2/(b2 - b1);
			b = 1 - 2*b2 / ( b2 - b1 );
			mouse.y =  a*event.y+b;
